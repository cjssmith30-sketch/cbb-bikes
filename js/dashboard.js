...
async function initOverview() {
  ...
  if (trendData && trendData.length) {
    makeChart('overviewRevenueChart', {
      ...
      data: {
        ...
        datasets: [{ label: 'Revenue', data: trendData.map(r => r.revenue), borderColor: ACCENT, backgroundColor: 'rgba(232,255,74,0.07)', borderWidth: 2.5, fill: true, tension: 0.4, pointRadius: 3, pointBackgroundColor: ACCENT }]
      },
      options: {
        responsive: true, maintainAspectRatio: true, aspectRatio: 3,
        plugins: { legend: { display: false } },
        scales: { x: { grid: { color: 'rgba(255,255,255,0.04)' } }, y: { grid: { color: 'rgba(255,255,255,0.04)' }, ticks: { callback: v => '$' + (v/1000).toFixed(0) + 'k' } } }
      }
    });
  }
  ...
  if (prodData && prodData.length) {
    ...
    makeChart('overviewMixChart', {
      type: 'doughnut',
      data: { labels: Object.keys(byType), datasets: [{ data: Object.values(byType), backgroundColor: PALETTE, borderColor: '#111', borderWidth: 2 }] },
    ...
    makeChart('overviewTopProducts', {
      type: 'bar',
      data: { labels: top5.map(p => p.Name.length > 14 ? p.Name.substring(0,14)+'…' : p.Name), datasets: [{ label: 'Units', data: top5.map(p => p.units_sold), backgroundColor: PALETTE, borderRadius: 4 }] },
      options: { indexAxis: 'y', responsive: true, plugins: { legend: { display: false } }, scales: { x: { grid: { color: 'rgba(255,255,255,0.04)' } }, y: { grid: { display: false } } } }
    });
  }
  ...
  if (empData && empData.length) {
    makeChart('overviewEmpChart', {
      type: 'bar',
      data: { labels: empData.slice(0,5).map(e => e.full_name?.split(' ')[0] || e.F_Name), datasets: [{ label: 'Revenue', data: empData.slice(0,5).map(e => e.total_revenue), backgroundColor: PALETTE, borderRadius: 4 }] },
      options: { responsive: true, plugins: { legend: { display: false } }, scales: { y: { ticks: { callback: v => '$'+(v/1000).toFixed(0)+'k' }, grid: { color: 'rgba(255,255,255,0.04)' } }, x: { grid: { display: false } } } }
    });
  }
}
...
function renderRevenueByProduct(data) {
  const filtered = activeProductType === 'all' ? data : data.filter(p => p.Type === activeProductType);
  if (!filtered.length) {
    setEmpty('revenueProductBody', 6, `No ${activeProductType} products sold in this period.`);
  ...
  makeChart('revenueProductChart', {
    ...
    data: {
      ...
      datasets: [{ label: 'Revenue', data: filtered.map(p => p.total_revenue), backgroundColor: filtered.map((_,i) => PALETTE[i % PALETTE.length]), borderRadius: 4 }]
    },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { display: false } },
      scales: { y: { ticks: { callback: v => '$'+(v/1000).toFixed(0)+'k' }, grid: { color: 'rgba(255,255,255,0.04)' } }, x: { grid: { display: false }, ticks: { maxRotation: 45 } } }
    }
  });
...
async function initMonthlyRevenueTrend() {
  ...
  makeChart('revenueTimeChart', {
    ...
    options: {
      ...
      scales: { y: { ticks: { callback: v => '$'+(v/1000).toFixed(0)+'k' }, grid: { color: 'rgba(255,255,255,0.04)' } }, x: { grid: { color: 'rgba(255,255,255,0.04)' } } }
    }
  });
  document.getElementById('revenueTimeChart').style.height = '340px';
...
async function initBestSelling() {
  ...
  makeChart('bestSellingChart', {
    type: 'bar',
    data: { labels: data.map(p => p.Name.length > 14 ? p.Name.substring(0,14)+'…' : p.Name), datasets: [{ label: 'Units Sold', data: data.map(p => p.units_sold), backgroundColor: PALETTE, borderRadius: 4 }] },
    options: { indexAxis: 'y', responsive: true, plugins: { legend: { display: false } }, scales: { x: { grid: { color: 'rgba(255,255,255,0.04)' } }, y: { grid: { display: false } } } }
  });
  makeChart('bestSellingRevChart', {
    type: 'doughnut',
    data: { labels: data.slice(0,6).map(p => p.Name.length > 14 ? p.Name.substring(0,14)+'…' : p.Name), datasets: [{ data: data.slice(0,6).map(p => p.revenue), backgroundColor: PALETTE, borderColor: '#111', borderWidth: 2 }] },
...
async function initEmpTransactions() {
  ...
  makeChart('empTransChart', {
    ...
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'top' } }, scales: { y: { grid: { color: 'rgba(255,255,255,0.04)' } }, x: { grid: { display: false } } } }
  });
  document.getElementById('empTransChart').style.height = '300px';
...
async function initEmpRevenue() {
  ...
  makeChart('empRevChart', {
    type: 'bar',
    data: { labels: data.map(e => e.full_name?.split(' ')[0] || e.F_Name), datasets: [{ label: 'Revenue', data: data.map(e => e.total_revenue), backgroundColor: PALETTE, borderRadius: 4 }] },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { ticks: { callback: v => '$'+(v/1000).toFixed(0)+'k' }, grid: { color: 'rgba(255,255,255,0.04)' } }, x: { grid: { display: false } } } }
  });
  document.getElementById('empRevChart').style.height = '300px';
...
async function initEmpAvg() {
  ...
  makeChart('empAvgChart', {
    ...
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'top' } }, scales: { y: { ticks: { callback: v => '$'+v.toLocaleString() }, grid: { color: 'rgba(255,255,255,0.04)' } }, x: { grid: { display: false } } } }
  });
  document.getElementById('empAvgChart').style.height = '300px';
...
async function initBWPByState() {
  ...
  makeChart('stateChart', {
    type: 'bar',
    data: { labels: data.map(r => r.state), datasets: [{ label: 'Participants', data: data.map(r => r.participant_count), backgroundColor: PALETTE, borderRadius: 4 }] },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { grid: { color: 'rgba(255,255,255,0.04)' }, ticks: { stepSize: 1 } }, x: { grid: { display: false } } } }
  });
  document.getElementById('stateChart').style.height = '300px';
...
async function initEventCounts() {
  ...
  makeChart('eventCountsChart', {
    type: 'bar',
    data: { labels: data.map(e => e.Name.length > 20 ? e.Name.substring(0,20)+'…' : e.Name), datasets: [{ label: 'Participants', data: data.map(e => e.participant_count), backgroundColor: ACCENT, borderRadius: 4 }] },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { grid: { color: 'rgba(255,255,255,0.04)' }, ticks: { stepSize: 1 } }, x: { grid: { display: false } } } }
  });
  document.getElementById('eventCountsChart').style.height = '300px';
...
async function initBWPRevenue() {
  ...
  makeChart('bwpRevPieChart', {
    type: 'doughnut',
    data: { labels: data.map(r => r.customer_group), datasets: [{ data: data.map(r => r.total_revenue), backgroundColor: [ACCENT, BLUE], borderColor: '#111', borderWidth: 2 }] },
  ...
  makeChart('bwpRevBarChart', {
    ...
    data: {
      ...
      datasets: data.map((r,i) => ({ label: r.customer_group, data: [r.customer_count, r.transaction_count, Math.round(r.avg_transaction)], backgroundColor: i === 0 ? 'rgba(232,255,74,0.7)' : 'rgba(74,159,255,0.6)', borderRadius: 4 }))
    },
    options: { responsive: true, plugins: { legend: { position: 'top' } }, scales: { y: { grid: { color: 'rgba(255,255,255,0.04)' } }, x: { grid: { display: false } } } }
  });
...
async function initDiscountImpact() {
  ...
  makeChart('discountChart', {
    ...
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'top' } }, scales: { y: { ticks: { callback: v => '$'+(v/1000).toFixed(0)+'k' }, grid: { color: 'rgba(255,255,255,0.04)' } }, x: { grid: { display: false } } } }
  });
  document.getElementById('discountChart').style.height = '300px';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('logoutBtn')?.addEventListener('click', () => {
    try { sessionStorage.removeItem('cbb_manager_auth'); sessionStorage.removeItem('cbb_manager_token'); } catch(e) {}
    window.location.href = 'manager-login.html';
  });

  // Date range — reset filter and reload
  ...
  // Product type filter — document-level delegation works even when section is hidden
